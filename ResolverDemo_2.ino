...
double setpoint = 180.0;  // Desired angle setpoint
...
void loop() {
  ...
  int pwmValue = 1500;
  if (output > 0){ 
    pwmValue = map(output, 0, 100, 1575, 2000);
  }
  if (output < 0){ 
    pwmValue = map(output, -100, 0, 1000, 1425);
  }
  if (output = 0){ 
    pwmValue = 1500;
  }
  ...
  avgError = total / numReadings;
  // send it to the computer as ASCII digits

if (avgError<5 && avgError>-5){
